 ...
 *
 *  3、对于高度不定的列表来说，需要在渲染完成后调用更新cell高度的方法
 *  manager.updateCellHeight([cellHeight1, cellHeight2, ...])
 *  可以通过下面的方式获取到cell的高度值
 *  $cell 为单个cell节点
 *    let height = $cell.getBoundingClientRect().height
 *
 *  4、对于高度不定的列表，初始渲染时是没有预估的整体高度的，需要在渲染完成后更新整个列表预估高度
 *  （这一步是体验优化，滚动条不可见的，可以忽略）
 *    // 这里其实只更新了scrollHeight, paddingBottom
 *    4.1 manager.updateScrollHeight()
 *    // 将预估高度渲染进去
 *    4.2 renderList = manager.getRenderInfo()
 *
 *  5、监听滚动列表容器的滚动事件，调用manager.updateScroll()进行动态列表更新
 *     然后重复执行 2 3 步
